<script lang="ts">
	...
	const form = superForm(data.form, {
		dataType: 'json',
		validators: zodClient(postCreateSchema),
	});
...
<svelte:head>
	<title>Karbasi.dev | Create Post</title>
</svelte:head>
...
<div class="mx-auto w-full">
	<form method="POST" use:enhance>
		...
		<Form.Field
			{form}
			name="active"
			class="flex flex-row items-start space-x-3 space-y-0 rounded-md border border-input p-4"
		>
			<Form.Control>
				{#snippet children({ props })}
